...
public class VariantEffectPredictor {
    ...
    public List<VariantEffect> run(Variant variant) {
        ...
        int endPos = variant.getPosition() + refBaseLength;
        String restCall = "/vep/human/region/" + variant.getChromosomeName() + ":" + variant.getPosition() + ":" + endPos + "/" + variant.getAlternateBase() + "?";
    ...
    private VariantEffect getVariantEffect(JSONObject input, JSONObject currentConseq, String consequence) {
        ...
        try {
            ...
            variantEffect.setTranscriptName(currentConseq.getString("transcript_id"));
